...
class IhrmEmpCURD(object):
    ...
    def add_emp(cls, header, json_data):
        ...
        token = header.json().get("data")
        print(token)
        headerr = {"Content-Type": "application/json;charset=UTF-8",
                   "Authorization": "Bearer " + token}
        print("cookies", header.cookies)
        respo = requests.post(url=url, headers=headerr, json=json_data, cookies=header.cookies)
        return respo
